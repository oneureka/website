...
const { data, isLoading } = useQuery({
  queryKey: ['user-following', login],
  queryFn: () => rest.users.listFollowing({ id: login.value as any }) as Promise<{ users: User[] }>,
})
  ...
  <UserProfileLayout>
    <Card>
      <div class="divide-y px-4">
        ...
        <EmptyState v-else-if="users.length === 0" message="暂无关注" />
        <RouterLink
          v-for="u in users"
          :key="u.id"
          :to="`/${u.login}`"
